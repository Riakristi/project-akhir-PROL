...
def getHarga(namaBarang):
    ...
    try:
        get = cursor.execute("SELECT Keterangan FROM data_barang WHERE Nama=?", (namaBarang,))
        fetch = get.fetchall()
        return fetch
...
def getVoucher(kode):
    """ sebuah fitur untuk mengambil kode voucher """
    try:
        get = cursor.execute("SELECT potongan FROM voucher WHERE kode=?", (kode,))
        fetch = get.fetchone()
        return fetch
